...
const Session = () => {
  ...
  const toggleActiveStatus = (row) => {
    let data = JSON.stringify({
      ...
      active_session: !row.original.active_session,
      session_name: row.original.session_name,
      session_year: row.original.session_year,
      day_start_time: row.original.day_start_time,
      day_end_time: row.original.day_end_time,
    });
        ...
        Cell: ({ row }) => (
          <label className="inline-flex items-center cursor-pointer">
            <input
              ...
              checked={row.original.active_session === 1}
