...
const Room = () => {
  ...
  const columns = useMemo(
    () => [
      {
        accessorKey: "room_id", //normal accessorKey
        header: "Room Id",
        size: 20,
      },
      {
  ...
  useEffect(() => {
    if (rows) {
      ...
      reset({
        ...
        room_name: rows.original.room_name,
        room_type: rows.original.room_type,
        room_capacity: rows.original.room_capacity,
  ...
  const onSubmit = async (data) => {
    if (rows) {
      try {
        const response = await axios.put("/api/database/room", data);
      ...
      try {
        const response = await axios.post("/api/database/room", data);
        console.log(response.data);
  ...
  const sessionApiUrl = "/api/database/room"; //where to post data
  ...
  return (
    <>
      ...
      <>
        <h1 className="text-3xl text-black mb-1 mt-5 font-extrabold">Room: </h1>
        <UploadData
