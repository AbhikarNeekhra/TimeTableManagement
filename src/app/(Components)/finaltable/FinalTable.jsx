...
const BranchTableFinal = (props) => {
  ...
  // console.log("props:: ",props.branchwiseprop.semester)

  let selectedBranchnew = props.branchwiseprop.branch;
  let selectedSessionnew = props.branchwiseprop.session;
  let selectedSemesternew = props.branchwiseprop.semester;
  if (
    ...
    selectedSessionnew != "" &&
    selectedSessionnew != " " &&
    selectedSemesternew != null &&
    selectedSemesternew != "" &&
    selectedSemesternew != " "
  ) {
    selectedBranchnew = selectedBranchnew.value;
    selectedSessionnew = selectedSessionnew.value;
    const selectedsemstername = selectedSemesternew.name;
    selectedSemesternew = selectedSemesternew.value;
    let selectedBranchDataContainer = document.getElementById(
      "selectedBranchDataContainer"
    );
    selectedBranchDataContainer.innerHTML = `<h3 >B.tech - ${selectedBranchnew} - Semster (${selectedsemstername}) </h3>`;
  ...
  const filteredDataWithSemester = selectedSemesternew
    ? filteredDataWithSession.filter(
      (entry) => entry.semester_id == selectedSemesternew
    )
    : data;
  //  console.log("filteredDataWithSEMESTER: ",filteredDataWithSemester)
  const filteredData = selectedBranchnew
    ? filteredDataWithSemester.filter(
      (entry) => entry.branch_id == selectedBranchnew
    )
    : data;
...
export default BranchTableFinal;
