...
const EditSemesterModal = ({ isOpen, onClose, rowData, onSave }) => {
  ...
  return (
    ...
    >
      <div className="modal-content bg-white p-8 rounded-md shadow-AbhiShad">
        <h2 className="text-lg font-bold mb-4">Edit Semester</h2>
        <div className="mb-4">
          <label className="block mb-1" htmlFor="semesterId">
            Semester ID
          </label>
          <input
            ...
            id="semesterId"
            name="semester_id"
            value={editedData.semester_id}
            onChange={handleChange}
            readOnly
          />
        </div>
        <div className="mb-4">
          <label className="block mb-1" htmlFor="semesterName">
            Semester Name
          </label>
          <input
            className="border border-gray-400 px-2 py-1 w-full"
...
const SemesterTable = () => {
  ...
  const deleteRow = async (row) => {
    try {
      const response = await axios.delete("/api/database/semester", {
                ...
                <table className="w-full ">
                  <thead>
                    <tr className="border border-solid shadow-md border-l-0 border-r-0">
                      <th className="text-md px-6 py-3">Semester ID</th>
                      <th className="text-md px-6 py-3">Semester Name</th>
