    ...
    <>
      ...
      <>
        <div>
                ...
                >
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      options={electivesubjectlist.map((SubjectSelect) => ({
                        value: {
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      options={faculty.map((facultyMember) => ({
                        value: {
                          email: facultyMember.faculty_email,
          ...
          <p className="text-3xl text-black mb-1 mt-3 font-extrabold ">
            Other Sessions Eg.(NEC/OC/LUNCH):
          </p>
          <form
            onSubmit={handleOtherSessionSubmit}
            className="flex flex-row flex-wrap"
          >
            <div>
              <label htmlFor="other_session_id">Name:</label>
              <select
                className="rounded-md border-2 m-2"
                // {...register("other_session_id")}
                required
                defaultValue={""}
              >
                ...
                {other_session.map((OtherSessionIDS) => (
                  <option
                    key={OtherSessionIDS.other_session_id}
                    // value={OtherSessionIDS.other_session_id}
                    value={OtherSessionIDS.other_session_name}
                  >
                    {`${OtherSessionIDS.other_session_name}`}
                  </option>
                ))}
              </select>
            </div>
            ...
            <div>
              ...
              <Select
                ...
                styles={{ "z-index": 17 }}
                value={semesterInput}
                onChange={(selectedOptionSemester) => {
                  setSemesterInput(selectedOptionSemester);
                  setSubjectInput(null);
                }}
                options={semester.map((SemesterSelect) => ({
                  value: SemesterSelect.semester_id,
                  name: SemesterSelect.semester_name,
                  label: `${SemesterSelect.semester_name}`,
                }))}
              />
            </div>
            <div>
              ...
              <Select
                ...
                styles={{ "z-index": 16 }}
                value={branchInput}
                onChange={(selectedOptionBranch) => {
                  setBranchInput(selectedOptionBranch);
                  // handleBranchOnChange(selectedOptionBranch);
                  // setSubjectInput(null);
                  setTimeSlotInput(null);
                }}
                options={branch.map((branchIDS) => ({
                  value: branchIDS.branch_id,
                  label: `${branchIDS.branch_id} ${branchIDS.branch_name}`,
                }))}
              />
            </div>
            ...
            <div>
              ...
              <Select
                ...
                styles={{ "z-index": 15 }}
                value={timeSlotInput}
                onChange={(selectedOptionSlotInput) => {
                  setTimeSlotInput(selectedOptionSlotInput);
                }}
              ...
              <div>
                ...
                <Select
                  ...
                  options={branch.map((branchID) => ({
                    ...
                    label: `${branchID.branch_id} ${branchID.branch_name}`,
                  }))}
                />
              </div>
              <div>
              ...
              <div>
                ...
                <Select
                  ...
                  onChange={(selectedOptionSlotInput) => {
                    ...
                    // handleTimeSlotOnChange(selectedOptionSlotInput);
                    setFacultyInput(null);
                    setRoomInput(null);
                    // setBranchInput(" ");
                  }}
                  options={filteredSlot.map((DayTimeSlot) => ({
                    value: DayTimeSlot.slot_id,
                    label: `${DayTimeSlot.day} ${DayTimeSlot.slot_start_time} - ${DayTimeSlot.slot_end_time}  `,
                  }))}
                />
              </div>
              ...
              <div>
                ...
                <Select
                  ...
                  options={filteredFaculty.map((facultyMember) => ({
                    value: {
                      email: facultyMember.faculty_email,
                      name: facultyMember.faculty_name,
                    },
